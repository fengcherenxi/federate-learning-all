...
def splitavg_propagation(arg, running_loss, chosen_net, generators, device, criterion, optimizers, optimizer_server):
	...
	if not arg.privacy_preserving:
    	# 如果是非隐私保护模式，则将label直接cat得到label_cat
	...
	# Forward propagation on client sub-net until the cut layer，前向传播过程
	...
	# Concatenate intermediate outputs at cut layer 聚合截断层输出特征
	...
	if not arg.privacy_preserving:
    	# 如果不是隐私保护模式，服务器端收到label，直接进行反向传播
