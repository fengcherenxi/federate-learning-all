...
def split_net(nets, server_net):
  # 输入nets是n个边缘设备的model数组，尾部是n维的特征向量，sever_net是服务器model
...
def setup_models(arg):# 初始化边缘设备和服务器设备
  nets = []
  acc_site, test_loss_site = {}
  for site in range(arg.site_num):# The total number of participating local sites
    if (arg.arch == "res34"):
